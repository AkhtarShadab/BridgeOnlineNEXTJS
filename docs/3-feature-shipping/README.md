...
| 02 | AI hint system | `feat/ai-hint-system` | `FEATURE_AI_HINTS` | ⏸️ Deferred | Backend stub only (`/hint` route returns 501). `lib/ai/` not built. Flag OFF in prod. |
| 03 | AI bot players | `feat/ai-bot-players` | `FEATURE_AI_BOTS` | ⏸️ Deferred | Depends on 02 |
| 04 | AI post-game summary | `feat/ai-postgame-summary` | `FEATURE_AI_SUMMARY` | ⏸️ Deferred | Depends on 02 |
| 05 | Playing table UI + multi-board | `feat/playing-table` | — | ✅ Merged to master via PR #22 | `PlayingTable.tsx`, scoreboard, auto-next |
...
| 12 | Hint button UI | `feat/hint-button` | `FEATURE_AI_HINTS` | ✅ Implemented `c9b2b6d` | HintButton component; UI shipped but **backend deferred with 02** — dormant while flag OFF |
| 14 | Games played counter & stats | `feat/games-played-counter` | — | ✅ Implemented | `lib/game/stats.ts`; per-board stats, winner-positive totalScore, dashboard Games Played + Win Rate |
...
P2 (polish):            11 → 12
Stats:                  14 (shipped)
AI (deferred):          02 → 03, 04   ← postponed; flags OFF in prod, no lib/ai/
```
